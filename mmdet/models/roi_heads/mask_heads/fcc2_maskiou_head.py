...
from mmcv.ops import Linear, MaxPool2d
...
class FCC2MaskIoUHead(nn.Module):
    """Mask IoU Head.
    ...
    def __init__(self,
                 ...
                 convwise=None):
        super(FCC2MaskIoUHead, self).__init__()
        ...
        if self.nl_stages[0]:
            # add nl layer
            self.nl_cfg['in_channels'] = in_channel
            self.fcs.append(build_plugin_layer(
                self.nl_cfg, '_fcs_pre')[1])
        for i in range(num_fcs):
            self.fcs.append(ConvModule(
                in_channel, in_channel, **self.convwise[0]))
            if len(self.convwise) > 1:
                self.fcs.append(ConvModule(
                    in_channel, out_channel, **self.convwise[1]))
        ...
        if self.nl_stages[1]:
            # add nl layer
            self.nl_cfg['in_channels'] = in_channel
            self.fcs.append(build_plugin_layer(self.nl_cfg, '_fcs')[1])

        fc_out_channels = in_channel * k_size * k_size
        self.fc_mask_iou = Linear(fc_out_channels, self.num_classes)
        self.relu = nn.ReLU()
